...
export function EditContractForm() {
    ...
    const [existedContract, setExistedContract] = useState();
    ...
    const getContract = async () => {
        const contract = await contractService.findObjById(contractId);
        await setExistedContract(contract);
    }
    ...
    const handleSubmit = async (value) => {
        const status = await contractService.edit(value);
        if (status === 200) {
            toast("Contract No#: " + value.name + " is edited!!");
            console.log(status);
            navigate("/contracts");
        } else {
